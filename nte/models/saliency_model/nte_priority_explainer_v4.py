...
from nte.experiment.utils import get_image, tv_norm, \
    ...
    backgroud_data_configuration, get_run_configuration, set_global_seed
...
class NTEPriorityGradientSaliencyV4(Saliency):
    def __init__(self, background_data, background_label, predict_fn, enable_wandb, use_cuda, args):
        super(NTEPriorityGradientSaliencyV4, self).__init__(background_data=background_data,
        ...
        self.priority_buffer = None  # PrioritizedBuffer(background_data=self.background_data)
        self.eps = 1.0
        self.eps_decay = 0.997
        self.mode="--"

    def greedy_priority_pick_zt(self, step):
        self.eps *= self.eps_decay
        if np.random.uniform() < self.eps:
            self.mode = 'Explore'
            P_index = [np.random.choice(len(self.background_data))]
            Zt, weight = [self.background_data[P_index[0]]], [1.0]
        else:
            self.mode = 'Exploit'
            Zt, weight, P_index = self.priority_buffer.sample(1)
        return torch.tensor(Zt[0], dtype=torch.float32), weight, P_index

    def priority_pick_zt(self, step, random=False):
        # if step < self.args.max_itr * 0.5:
        if random:
            P_index = [np.random.choice(len(self.background_data))]
            Zt, weight = [self.background_data[P_index[0]]], [1.0]
        else:
    ...
    def initialize_priorities(self, data, category, mse_loss_fn, kwargs):
        print("====" * 40)
        print("Finding initial valus for Zt priorities")
        mask = torch.tensor(torch.nn.MSELoss(reduction='none')(data, torch.tensor(
            self.args.dataset.__getattribute__(f'test_class_{1 - category}_mean'),
        ...
        for i in tqdm.tqdm(range(self.args.max_itr)):
            upsampled_mask = mask
            Zt, weight, P_index = self.priority_pick_zt(i, random=True)
            ...
            c1 = 1000000 * mse_loss_fn(outputs, kwargs['target'])
            ...
            loss = (c1 + c2 + c3)  # * weight[0]
            prios = loss
            metrics['Mean Gradient'].append(0.0)
            metrics['TV Norm'].append(float(c3.item()))
            metrics['MSE'].append(float(c1.item()))
            metrics['DIST'].append(0.0)
            ...
            print(
                ...
                f"| B: {metrics['Budget'][-1]:.4f} | W:{weight[0]:.2f} | TL: {metrics['Total Loss'][-1]:.4f}"
                f"| S:{metrics['Saliency Sum'][-1]:.4f}"
                f"| RI: {P_index[0]}"
                # f"| DTW: {c4.item():.2f}"
                f"")
            self.priority_buffer.initialize_priorities(P_index, [prios.item()])
        ...
        self.priority_buffer.finalize_priorities()
        print(f"Zt priorities:\n {self.priority_buffer.priorities}")
        print(
            f"Zt probs:\n {sorted(torch.nn.Softmax(dim=-1)(torch.tensor(self.priority_buffer.priorities, dtype=torch.float32)).numpy())}")
        print("====" * 40)
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        self.priority_buffer = PrioritizedBuffer(background_data=self.background_data)
        self.eps = 1.0
        category = np.argmax(kwargs['target'].cpu().data.numpy())

        mse_loss_fn = torch.nn.MSELoss(reduction='mean')
        # kl_loss_fn = torch.nn.KLDivLoss(size_average=False)
        cs_loss_fn = torch.nn.CosineSimilarity(dim=-1)
        softdtw_loss_fn = SoftDTW()

        # self.initialize_priorities(data, category, mse_loss_fn, kwargs)
        ...
        for i in tqdm.tqdm(range(self.args.max_itr)):
            ...
            Zt, weight, P_index = self.greedy_priority_pick_zt(i)
            ...
            # c1 = self.args.mse_coeff * (kl_loss_fn(kwargs['target'].log(), outputs))
            c2 = self.args.l1_coeff * torch.mean(mask) * float(self.args.enable_budget)
            c3 = self.args.tv_coeff * tv_norm(mask, self.args.tv_beta) * float(self.args.enable_tvnorm)
            # t1 = torch.tensor(1.0) * (torch.clamp(cs_loss_fn(perturbated_input, Zt), 0, 1))
            # t1_1 = torch.tensor(1.0) * (1-cs_loss_fn(perturbated_input, Zt))
            # t2 = torch.tensor(1.0) * (torch.clamp(cs_loss_fn(data, Zt), 0, 1))
            # t2_1 = torch.tensor(1.0) * (1-cs_loss_fn(data, Zt))
            # t3 = torch.tensor(1.0) * (mse_loss_fn(Zt, torch.tensor(self.args.dataset.__getattribute__(f'test_class_{category}_mean'),
            #                                          dtype=torch.float32)))
            # t3_1 = torch.tensor(-1.0) * (
            #     mse_loss_fn(Zt, torch.tensor(self.args.dataset.__getattribute__(f'test_class_{category}_mean'),
            #                                  dtype=torch.float32)))
            # t3 = torch.tensor(1.0) * (mse_loss_fn(perturbated_input, torch.tensor(
            #     self.args.dataset.__getattribute__(f'test_class_{category}_mean'),
            #     dtype=torch.float32)))
            mse_var = np.var(metrics['MSE']) if len(metrics['MSE']) > 1 else 0.0
            ...
            loss = (c1 + c2 + c3 + dist_loss) * weight[0]
            prios = loss + 1e-5  # +t2_1
            optimizer.zero_grad()
            loss.backward()
            ...
            print(
                ...
                f"| B: {metrics['Budget'][-1]:.4f} | W:{weight[0]:.2f} | TL: {metrics['Total Loss'][-1]:.4f}"
                f"| S:{metrics['Saliency Sum'][-1]:.4f}"
                f"| RI: {P_index[0]} | EPS: {self.eps:.2f} | M: {self.mode}"
                # f"| DTW: {c4.item():.2f}"
                f"")

            optimizer.step()
            # self.priority_buffer.update_priorities(P_index, [metrics['Total Loss'][-2]-prios.item() if i>2 else prios.item()])
            self.priority_buffer.update_priorities(P_index, [prios.item()])
