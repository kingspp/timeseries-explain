...
class NTEPriorityGradientSaliencyV2(Saliency):
    def __init__(self, background_data, background_label, predict_fn, enable_wandb, use_cuda, args):
        super(NTEPriorityGradientSaliencyV2, self).__init__(background_data=background_data,
    ...
    def priority_pick_zt(self):
        priors = self.priority.cpu().detach().numpy()
        # priors = priors.max() - priors + priors.min()
        priors = self.softmax_fn(torch.tensor(priors, dtype=torch.float32)).cpu().detach().numpy()
        P_index = np.random.choice(len(self.background_data), 1, p=priors)[0]
        print(self.priority)
        # P_index = np.argmax(priors)
    ...
    def generate_saliency(self, data, label, **kwargs):
        self.priority = torch.nn.Parameter(
            # torch.rand(size=[len(self.background_data)]),
            torch.empty(len(self.background_data)).fill_(1e-3),
            requires_grad=True)
        ...
        for i in tqdm.tqdm(range(self.args.max_itr)):
            ...
            Zt, P_index = self.priority_pick_zt()
            ...
            if self.args.enable_dist:
                if self.args.dist_loss == 'euc':
                    ...
                    dist_loss = self.args.dist_coeff * softdtw_loss_fn(x=perturbated_input.reshape([1, -1]),
                                                                       ...
                                                                       normalize=True,
                                                                       w=upsampled_mask.reshape([1, -1]))
            c5 = self.priority[P_index]
            loss = (c1 + c2 + c3 + dist_loss ) * c5
            optimizer.zero_grad()
            loss.mean().backward()
            ...
            print(
                ...
                # f"| PW: {metrics['PW'][-1]}  "
                f"| IDX: {P_index} | W: {c5.item():.4f} | S:{metrics['Saliency Sum'][-1]:.4f}"
