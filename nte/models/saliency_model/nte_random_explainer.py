...
class NTERandomSaliency(Saliency):
    def __init__(self, background_data, background_label, predict_fn, enable_wandb, use_cuda, args):
        super(NTERandomSaliency, self).__init__(background_data=background_data, background_label=background_label,
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        metrics = {"TV Norm": [], 'TV Coeff': [], "MSE": [], "Budget": [], "Total Loss": [],
                   "Confidence": [],
                   "Saliency Var": [],
                   "Saliency Sum": [], "MSE Coeff": [], "L1 Coeff": [],
                   "Category": [], "DTW": [], "DTW Coeff": [], "epoch": {}, 'MSE Var': [], 'EUC':[]}
        ...
        for i in tqdm.tqdm(range(self.args.max_itr)):
            ...
            if self.args.enable_euclidean_loss:
                c5 = mse_var * mse_loss_fn(data.reshape([1, -1]), perturbated_input.reshape([1, -1])) * 1.5
                # c5 = mse_var * softdtw_loss_fn(x=perturbated_input.reshape([1, -1]),
                #                                        y=data.reshape([1, -1]))
            ...
            metrics['EUC'].append(float(c5.item()))
            ...
            mask = torch.rand(size=mask.shape)
            # optimizer.step()
            #
            # if self.args.enable_lr_decay:
