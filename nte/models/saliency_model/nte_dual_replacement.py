...
class NTEDualReplacementGradientSaliency(Saliency):
    def __init__(self, background_data, background_label, predict_fn, enable_wandb, use_cuda, args):
        super(NTEDualReplacementGradientSaliency, self).__init__(background_data=background_data,
    ...
    def dynamic_dual_pick_zt(self, kwargs, data, label):
        ds = getattr(kwargs['dataset'], f"test_class_{int(label)}_indices")
        ls = len(ds)
        ods = getattr(kwargs['dataset'], f"test_class_{1-int(label)}_indices")
        ols = len(ods)
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        for i in (range(self.args.max_itr)):
            ...
            Zt, ZOt = self.dynamic_dual_pick_zt(kwargs=kwargs, data=data, label=label)
            Rt = []
            Rm = []

            for e, (zt, zot, m) in enumerate(zip(Zt.flatten(), ZOt.flatten(), mask.detach().numpy().flatten())):
                if m < 0:
                    # Rt.append(data[e])
                    Rt.append(Zt[e])
                    # Rt.append(ZOt[e])
