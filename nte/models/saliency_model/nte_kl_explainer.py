...
class NTEGradientKLSaliency(Saliency):
    def __init__(self, background_data, background_label, predict_fn, enable_wandb, use_cuda, args):
        super(NTEGradientKLSaliency, self).__init__(background_data=background_data, background_label=background_label,
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        metrics = {"TV Norm": [], 'TV Coeff': [], "KL": [], "Budget": [], "Total Loss": [],
                   "Confidence": [],
                   "Saliency Var": [],
                   "Saliency Sum": [], "KL Coeff": [], "L1 Coeff": [], "Mean Gradient": [],
                   "Category": [],
                   # "DTW": [], "DTW Coeff":[], 'EUC': [],
                   "epoch": {}, 'KL Var': [], 'DIST': []}
        ...
        for i in tqdm.tqdm(range(self.args.max_itr)):
            ...
            # print(outputs.shape, kwargs['target'].shape)
            # print(torch.sum(outputs), torch.sum(kwargs['target']))
            # print()
            # print(F.kl_div(outputs.log(), kwargs['target'], None, None, 'sum'))
            # print(outputs, kwargs['target'])
            # exit()
            ...
            kl_var = np.var(metrics['KL']) if len(metrics['KL']) > 1 else 0.0
            ...
            metrics['KL'].append(float(c1.item()))
            ...
            metrics['KL Coeff'].append(self.args.mse_coeff)
            ...
            print(
                ...
                f"| KL: {metrics['KL'][-1]:.4f}, V:{kl_var:.4f}"
