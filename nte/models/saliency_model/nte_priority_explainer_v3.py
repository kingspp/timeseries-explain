...
class NTEPriorityGradientSaliencyV3(Saliency):
    def __init__(self, background_data, background_label, predict_fn, enable_wandb, use_cuda, args):
        super(NTEPriorityGradientSaliencyV3, self).__init__(background_data=background_data,
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        optimizer = torch.optim.Adam([{'params': mask, 'lr': self.args.lr},
                                      {'params': self.priority, 'lr': self.args.lr}],
        ...
        for i in tqdm.tqdm(range(self.args.max_itr)):
            ...
            print(
                ...
                # f"| PW: {metrics['PW'][-1]}  "
                f"|EPS:{self.eps:.4f} | IDX: {P_index} | W: {c5.item():.4f} | M: {self.mode[-3:]} | S:{metrics['Saliency Sum'][-1]:.2f}"
