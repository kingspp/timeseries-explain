...
class NTEPriorityGradientSaliencyV1(Saliency):
    def __init__(self, background_data, background_label, predict_fn, enable_wandb, use_cuda, args):
        super(NTEPriorityGradientSaliencyV1, self).__init__(background_data=background_data,
    ...
    def priority_pick_zt(self):
        p_zt, p_index = self.memory.sample()
        return torch.tensor(p_zt, dtype=torch.float32), p_index
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        for i in tqdm.tqdm(range(self.args.max_itr)):
            ...
            print(
                ...
                # f"| PW: {metrics['PW'][-1]}  "
                f"| Prios: {float(prios):.4f} | IDX: {P_index}"
