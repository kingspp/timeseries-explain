...
from nte.models.saliency_model.cm_w_replacement_gradient_saliency import CMWRGradientSaliency
...
from nte.models.saliency_model.nte_priority_explainer import NTEPriorityGradientSaliency
from nte.models.saliency_model.nte_priority_explainer_v1 import NTEPriorityGradientSaliencyV1
from nte.models.saliency_model.nte_priority_explainer_v2 import NTEPriorityGradientSaliencyV2
from nte.models.saliency_model.nte_priority_explainer_v3 import NTEPriorityGradientSaliencyV3
from nte.models.saliency_model.nte_priority_explainer_v4 import NTEPriorityGradientSaliencyV4
from nte.models.saliency_model.nte_priority_explainer_v5 import NTEPriorityGradientSaliencyV5
from nte.models.saliency_model.nte_dual_replacement import NTEDualReplacementGradientSaliency
from nte.models.saliency_model.nte_kl_explainer import NTEGradientKLSaliency
from nte.models.saliency_model.ts_pert_explainer import TSPertSaliency
import random
...
if __name__ == '__main__':
    ...
    model = get_model(dest_path=args.bbm_path, dataset=args.dataset, use_cuda=use_cuda, bbm=args.bbm)
    ...
    if args.algo == 'cm':
        cm = CMGradientSaliency(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len], predict_fn=model,
                                enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'cmwr':
        cm = CMWRGradientSaliency(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len], predict_fn=model,
                                  enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'mse':
        mse = MSEGradientSaliency(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len], predict_fn=model,
                                  enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'nte':
        nte = NTEGradientSaliency(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len], predict_fn=model,
                                  enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'nte-dual':
        nte_dual = NTEDualReplacementGradientSaliency(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len], predict_fn=model,
                                  enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'ts-pert':
        ts_pert = TSPertSaliency(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len], predict_fn=model,
                                  enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'nte-kl':
        nte_kl = NTEGradientKLSaliency(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len], predict_fn=model,
                                  enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'p-nte':
        pnte = NTEPriorityGradientSaliency(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len],
                                           predict_fn=model,
                                           enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'p-nte-v1':
        pnte_v1 = NTEPriorityGradientSaliencyV1(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len],
                                                predict_fn=model,
                                                enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'p-nte-v2':
        pnte_v2 = NTEPriorityGradientSaliencyV2(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len],
                                                predict_fn=model,
                                                enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'p-nte-v3':
        pnte_v3 = NTEPriorityGradientSaliencyV3(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len],
                                                predict_fn=model,
                                                enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'p-nte-v4':
        pnte_v4 = NTEPriorityGradientSaliencyV4(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len],
                                                predict_fn=model,
                                                enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
    elif args.algo == 'p-nte-v5':
        pnte_v5 = NTEPriorityGradientSaliencyV5(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len],
                                                predict_fn=model,
                                                enable_wandb=ENABLE_WANDB, args=args, use_cuda=use_cuda)
        ...
        lime = LimeSaliency(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len], predict_fn=model)
    elif args.algo == 'shap':
        shap = SHAPSaliency(background_data=bg_data[:bg_len], background_label=bg_data[:bg_len], predict_fn=model)
    elif args.algo == 'rise':
        ...
        try:
            ...
            if args.algo == "mse":
                if args.background_data == "none":
                    mse.background_data = original_signal
                    mse.background_label = original_label
                mask, perturbation_manager = mse.generate_saliency(
            ...
            if args.algo == "nte":
                ...
                mask, perturbation_manager = nte.generate_saliency(
                    data=original_signal, label=original_label,
                    save_dir=SAVE_DIR, save_perturbations=args.save_perturbations, target=target, dataset=dataset)
                mask = mask.flatten()
                ...
                if args.background_data == "none":
                    nte.background_data = original_signal
                    nte.background_label = original_label
                mask, perturbation_manager = nte_dual.generate_saliency(
                    data=original_signal, label=original_label,
                    save_dir=SAVE_DIR, save_perturbations=args.save_perturbations, target=target, dataset=dataset)
                mask = mask.flatten()
            elif args.algo == "ts-pert":
                if args.background_data == "none":
                    nte.background_data = original_signal
                    nte.background_label = original_label
                mask, perturbation_manager = ts_pert.generate_saliency(
                    data=original_signal, label=original_label,
                    save_dir=SAVE_DIR, save_perturbations=args.save_perturbations, target=target, dataset=dataset)
                mask = mask.flatten()
            elif args.algo == "nte-kl":
                if args.background_data == "none":
                    nte.background_data = original_signal
                    nte.background_label = original_label
                mask, perturbation_manager = nte_kl.generate_saliency(
                    data=original_signal, label=original_label,
                    save_dir=SAVE_DIR, save_perturbations=args.save_perturbations, target=target, dataset=dataset)
                mask = mask.flatten()
            elif args.algo == "p-nte":
                if args.background_data == "none":
                    nte.background_data = original_signal
                    nte.background_label = original_label
                mask, perturbation_manager = pnte.generate_saliency(
                    data=original_signal, label=original_label,
                    save_dir=SAVE_DIR, save_perturbations=args.save_perturbations, target=target, dataset=dataset)
                mask = mask.flatten()
                ...
                if args.background_data == "none":
                    nte.background_data = original_signal
                    nte.background_label = original_label
                mask, perturbation_manager = pnte_v1.generate_saliency(
                    data=original_signal, label=original_label,
                    save_dir=SAVE_DIR, save_perturbations=args.save_perturbations, target=target, dataset=dataset)
                mask = mask.flatten()
                ...
                if args.background_data == "none":
                    nte.background_data = original_signal
                    nte.background_label = original_label
                mask, perturbation_manager = pnte_v2.generate_saliency(
                    data=original_signal, label=original_label,
                    save_dir=SAVE_DIR, save_perturbations=args.save_perturbations, target=target, dataset=dataset)
                mask = mask.flatten()
                ...
                if args.background_data == "none":
                    nte.background_data = original_signal
                    nte.background_label = original_label
                mask, perturbation_manager = pnte_v3.generate_saliency(
                    data=original_signal, label=original_label,
                    save_dir=SAVE_DIR, save_perturbations=args.save_perturbations, target=target, dataset=dataset)
                mask = mask.flatten()
                ...
                if args.background_data == "none":
                    nte.background_data = original_signal
                    nte.background_label = original_label
                mask, perturbation_manager = pnte_v4.generate_saliency(
                    data=original_signal, label=original_label,
                    save_dir=SAVE_DIR, save_perturbations=args.save_perturbations, target=target, dataset=dataset)
                mask = mask.flatten()
                ...
                if args.background_data == "none":
                    nte.background_data = original_signal
                    nte.background_label = original_label
                mask, perturbation_manager = pnte_v5.generate_saliency(
                    data=original_signal, label=original_label,
                    save_dir=SAVE_DIR, save_perturbations=args.save_perturbations, target=target, dataset=dataset)
                mask = mask.flatten()
                ...
                if args.background_data == "none":
                    nte_rand.background_data = original_signal
                    nte_rand.background_label = original_label
                ...
                if args.background_data == "none":
                    cm.background_data = original_signal
                    cm.background_label = original_label
                ...
                if args.background_data == "none":
                    lime.background_data = original_signal
                    lime.background_label = original_label
                ...
                if args.background_data == "none":
                    shap.background_data = original_signal
                    shap.background_label = original_label
            ...
            if args.run_eval:
                ...
                json.dump(metrics, open(SAVE_DIR + "/metrics.json", 'w'), indent=2)

            if args.save_perturbations:
                perturbation_manager.to_csv(SAVE_DIR=SAVE_DIR, TAG=TAG, UUID=UUID, SAMPLE_ID=cur_ind)
                if ENABLE_WANDB:
                    wandb.save(f"{SAVE_DIR}/perturbations-{TAG}-{UUID}-{cur_ind}.csv")
