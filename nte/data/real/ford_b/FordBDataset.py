...
class FordBDataset(Dataset):
    def __init__(self):
        super().__init__(
            name='FordB',
            meta_file_path=os.path.join(NTE_MODULE_PATH, 'data', 'real', 'ford_b', 'meta.json'))

    def load_train_data(self):
        df = pd.read_csv(os.path.join(NTE_MODULE_PATH, 'data', 'real', 'ford_b', 'train.csv'))
    ...
    def load_test_data(self):
        df = pd.read_csv(os.path.join(NTE_MODULE_PATH, 'data', 'real', 'ford_b', 'test.csv'))
        df = df.drop(['id'], axis=1)
